...
Apple 앱·프레임워크 표면, 로컬 거버넌스·컨텍스트, opt-in 외부 통합을 아우르는 32개 모듈을 하나의 MCP 인터페이스로 열어, 어떤 AI 클라이언트든 내 Apple 데이터 위에서 읽고·쓰고·행동할 수 있게 한다. "오늘 일정 정리해 줘"가 아니라 **"지난주 회의 메모에서 실행항목 뽑아 리마인더로 옮기고, 완료되면 알려줘"**가 되는 경험 — 그게 이 도구가 여는 지점이다. (AirMCP가 "기억하거나 계획"하는 게 아니라, AirMCP가 노출한 맥락 위에서 AI가 그렇게 할 수 있게 한다.)
...
- **오픈소스에 기여하거나 포크해서 쓰고 싶은 개발자.** TypeScript, 32개 모듈, 확장 1~2파일.
...
```
Approval-gated local action runtime for Apple workspaces.
Profile-based modules, progressive MCP tool exposure, Skills DSL workflow engine,
semantic memory, OAuth 2.1, HMAC-chained audit log
...
| GitHub README·개발자 문서 (`docs/site/`) | 건조·정확·런타임 레이어 강조 | MCP 서버를 포크·확장할 개발자 | "Approval-gated local action runtime for Apple workspaces. 296 tools / 32 modules behind profiles + progressive exposure + Skills DSL + semantic memory + OAuth 2.1 + HMAC audit log." |

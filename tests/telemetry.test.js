...
describe("traceApproval (no OTel SDK installed)", () => {
  ...
  test("resolves without error for timeout and unavailable decisions", async () => {
    await expect(traceApproval("slow_write", "timed_out", "socket")).resolves.toBeUndefined();
    await expect(traceApproval("offline_write", "unavailable", "unavailable")).resolves.toBeUndefined();
  });

  test("resolves without error for skipped decision", async () => {
    await expect(traceApproval("read_note", "skipped", "socket", { managed: true })).resolves.toBeUndefined();
  });

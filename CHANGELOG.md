...
### Fixed
- **`AskAirMCPIntent` rejects empty / whitespace-only prompts** ([`swift/Sources/AirMCPKit/AskAirMCPIntent.swift`](swift/Sources/AirMCPKit/AskAirMCPIntent.swift)) — previously a Siri "Ask AirMCP" invocation with no follow-up text (a common failure mode when the user pauses) hit `LanguageModelSession.respond(to: "")` which throws an opaque framework error. Now we trim and return a recoverable user-facing message ("Please ask a question — I need something to look up.") so Siri / Shortcuts surfaces something the user can act on instead of a generic "intent failed".
- **`gen-llms-txt.mjs` count drift fully closed** ([`scripts/gen-llms-txt.mjs`](scripts/gen-llms-txt.mjs)) — the script was reporting `265 tools / 32 modules` while `count-stats.mjs` (canonical) reported `269 / 29`. Two roots: (1) `extractTools`'s strict regex (requiring co-located `title:` + `description:`) missed 4 tools whose registration spans the regex boundary; (2) `walkDir` grouped every src/ subdir with a `registerTool` call into a "module", overcounting cross/semantic/skills/server/shared as separate modules. Fix: a separate broad-regex pass (`/server\.registerTool\(/g`) computes the totals (matching `count-stats`'s methodology), and the canonical module count is parsed from `src/shared/config.ts` `MODULE_NAMES`. The per-module list keeps using the strict regex for the rich `tool name + title + description` rendering in `llms-full.txt`. Headline now reads `269 tools across 29 modules` everywhere.
- **`safari` and `podcasts` modules carry `compatibility.deprecation` for macOS 26** ([`src/shared/modules.ts`](src/shared/modules.ts)) — RFC 0004 G-5. Apple removed Safari `make new bookmark` JXA verb (just the one tool, replaced by `add_to_reading_list`) and the entire Podcasts JXA dictionary in macOS 26. Both now declare `compatibility.brokenOn: [26]` + `deprecation` blocks so RFC 0004's `print-compat-report` and `airmcp doctor` surface the regression with the exact replacement (Safari) or upstream-removal note (Podcasts) instead of leaving the user to debug runtime errors.
...
- **Tool count drift in user-facing docs** — `llms.txt` said `258 tools across 30 modules` (regenerated to current via `npm run llms`); `docs/shortcuts.md` said `154 read-only tools` / `144 intents` (now correctly 229 tools / 219 non-pinned intents reflecting RFC 0007 Phase A's full write-capable surface). The deeper discrepancy between `count-stats.mjs` (29 modules / 269 tools — canonical from `MODULE_NAMES`) and `gen-llms-txt.mjs` (32 modules / 265 tools — walks `src/` for any dir with `registerTool`) is a separate script-reconciliation issue tracked for a follow-up PR; both numbers are now closer to truth than what shipped.

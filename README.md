...
**Open action runtime for Apple-native agents.** Skills DSL workflow engine, semantic memory, OAuth 2.1, HMAC-chained audit log — over *optional* native Swift bridges into EventKit, HealthKit, PhotoKit, Vision, and Foundation Models. 272 tools across 29 Apple + Google Workspace modules (a curated **starter** set loads by default; `--full` enables all). Connect Claude, Codex, opencode, Gemini CLI, Antigravity, Cursor, Zed, Cline, JetBrains Air, OpenClaw, and Xcode 27 agents (which speak MCP since 26.3) — any MCP-capable AI.

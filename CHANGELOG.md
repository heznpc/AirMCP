...
### Distribution
- **Add-on package staging + lazy harness split** — `npm run addons:build` now stages tarball-ready physical add-on packages for every non-core module pack, and `npm run addons:check` is wired into CI/release preflight. Runtime module loading now prefers installed add-on packages (`AIRMCP_ADDON_PACKAGE_MODE=prefer-installed`) before bundled fallback, with `external-only` available as a strict package-boundary probe. Tool discovery now returns compact descriptions by default and exposes `describe_tool` for full per-tool detail on demand. Task-session policy moved behind harness adapters (`compatible`, `strict`, `app-runtime`, `agent`) so client-specific behavior is no longer hard-coded inside server setup.
- **Module runtime hardening** — `npx airmcp modules` now lists, enables, disables, and doctors DLC-like module add-ons using the same `modulePacks` config consumed by the runtime. `npm run tokens:check` adds a CI budget for tool-description token drift, `profiles:check` now includes extra restricted-pack wire cases plus discovery golden queries, and newly generated app/CLI configs set `requireToolSession: true` so hidden tools flow through task-scoped sessions by default while the no-config stdio path stays compatible.
- **DLC-like module pack contract** — new `src/shared/module-packs.ts` defines runtime packs (`core`, `communications`, `productivity`, `browser`, `media`, `visual`, `location`, `device`, `intelligence`, `google-workspace`, `spatial`) and their future add-on package names without a `pack-` prefix (`airmcp`, `@heznpc/airmcp-productivity`, `@heznpc/airmcp-spatial`, ...). `AIRMCP_MODULE_PACKS` / `config.json -> modulePacks` can now restrict the available pack set while always keeping `core`; module loading skips unavailable-pack modules before dynamic import. New `list_module_packs` exposes pack status over MCP, `profile_status` reports `modulePacksConfigured`, `modulePacksAvailable`, and `modulesMissingPacks`, and `profiles:check` includes a real MCP wire case proving `core,productivity` keeps iWork while marking communications modules as missing-pack.

...
- [ ] **RFC 0007 Draft — MCP Tool → App Intent/AppEntity codegen** (next, see §15.1)
...
| 도구 수       | 270+ (run-time 등록 ~111; smoke 기준)               | ~80–100 (Phase 1: ~40, Health 확장 후 ~60)                 |
| 배포          | npm (`npx airmcp`)                                  | App Store                                                  |

**핵심 전략**: 공유 Swift Package(`AirMCPKit`)를 extract하여 macOS/iOS 양쪽에서 사용. **Phase 2부터는 MCP 툴 메타데이터를 App Intent로 자동 생성** (RFC 0007) → Siri/Shortcuts/Spotlight/App Schemas 표면과 MCP HTTP 표면을 같은 계약에서 유지한다. Phase 3에서 **FoundationModels 로 자체 에이전트 루프** 내장.
